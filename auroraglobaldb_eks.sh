...
function install_ascp_provider()
{
  print_line
  echo "Installing AWS Secrets Manager Configuration Provider, Kubernetes Secrets Store CSI Driver"
  print_line
  ...
  eksctl create iamserviceaccount \
    ...
    --attach-policy-arn=arn:aws:iam::${AWS_ACCOUNT_ID}:policy/eks-secretsmanager-policy \
...
function install_pgbouncer()
{
    ...
    dbuser=`echo "$secret" | jq '.username' | sed -e 's/"//g'`
    dbpass=`echo "$secret" | jq '.password' | sed -e 's/"//g'`

    dbusersecret=$(aws secretsmanager get-secret-value --region ${AWS_REGION} --secret-id aurora-pg/dbuser1/EKSGDB1 --query SecretString --output text)
    DBUSER=`echo "$dbusersecret" | jq '.username' | sed -e 's/"//g'`
    DBPASSWD=`echo "$dbusersecret" | jq '.password' | sed -e 's/"//g'`
    ...
    if [[ -z ${DBUSER} || -z ${DBPASSWD} ]]; then
	    echo "ERROR: Failed to get db user password from secrets manager"
	    exit 1
    fi

    if [[ $runscript -eq 1 ]]; then
        export PGPASSWORD=$dbpass
        psql -h $rwendpoint -U $dbuser -p 5432 -d eksgdbdemo -v dbuser="${DBUSER}" -v dbpasswd="${DBPASSWD}" -v database="eksgdbdemo" -f retailapp/sql/setup_schema.sql
    fi
...
function update_c9()
{
  ...
  for region in "${REGION1}" "${REGION2}"; do
    ...
    EnvironmentID=`echo $cft | awk -F'/' '{print $(NF)}'`
    echo $region:  $cft: $EnvironmentID
    aws cloud9 create-environment-membership --environment-id $EnvironmentID --user-arn $userarn --permissions read-write --region $region
  done
}
